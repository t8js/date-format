...
for (let m = 0; m < months.length; m++) {
    ...
    is(formatDate(new Date(2022, m, 1), '{MMM} {D}', transform), `${months[m]} 1`);
}

for (let wd = 0; wd < weekDays.length; wd++)
    is(formatDate(new Date(2022, 6, 10 + wd), '{WD} {M}/{DD}', transform), `${weekDays[wd]} 7/${10 + wd}`);
is(formatDate('2022-07-21T12:34:56.789', '{WD} {M}/{DD}', transform), 'Thu 7/21');
is(formatDate('2022-12-02T12:34:56.789', '{WD}, {MMM} {D}, {Y}', transform), 'Fri, Dec 2, 2022');
...
suite('era');
let eraTemplate: FormatDateTemplate = ({E}) => E === 1 ? 'AD {YE}' : '{YE} BC';
let commonEraTemplate: FormatDateTemplate = ({CE}) => CE === 1 ? '{YE} CE' : '{YE} BCE';
